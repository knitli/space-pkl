...
for config_type in project workspace toolchain template; do
    ...
    if [ -f "examples/${config_type}.yml" ]; then
        ...
        if ./target/release/spklr convert \
            ...
            if ./target/release/spklr convert \
                ...
                if validate_file_content "test_output/${config_type}_roundtrip.yml" "# Generated by spklr\|language:\|type:\|projects:\|node:\|title:"; then
...
for config_type in project workspace toolchain template; do
    ...
    if ./target/release/spklr generate schema \
        --config-type $config_type \
        --format json-schema \
...
# Test CLI help
if ./target/release/spklr --help &>/dev/null; then
...
# Test CLI version
if ./target/release/spklr --version &>/dev/null; then
...
**CLI Version:** $(./target/release/spklr --version 2>/dev/null || echo "Unknown")
