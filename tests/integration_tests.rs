...
use moon_config_cli::pkl_tooling::*;
use moon_config_cli::config_processor::*;
    ...
    if let Ok(Some(pkl_cli)) = find_pkl_executable().await {
        ...
        match report {
            Ok(compatibility_report) => {
                ...
                println!("Basic functionality: {}", compatibility_report.basic_functionality);
                println!("Moon config integration: {}", compatibility_report.moon_config_integration);
                println!("Extend/amend support: {}", compatibility_report.extend_amend_support);
                println!("Schema generation: {}", compatibility_report.schema_generation);
                ...
                if compatibility_report.basic_functionality {
                    ...
                    println!("⚠️  Basic Pkl functionality not yet fully implemented (expected in development)");
                }
            }
            Err(e) => {
...
async fn test_pkl_version_management() {
    ...
    assert!(compatible_versions.contains(&recommended_version),
           "Recommended version should be in compatible versions list");

    // Check version format
    assert!(recommended_version.matches('.').count() == 2,
...
async fn test_enhanced_error_handling() {
    use moon_config_cli::error::{CliError, validation_error};
...
async fn test_error_handling_with_miette() {
    use moon_config_cli::error::CliError;
    use std::path::PathBuf;
...
async fn test_comprehensive_cli_integration() {
    ...
    use moon_config_cli::pkl_tooling::{get_recommended_pkl_version, get_compatible_pkl_versions};

    let recommended = get_recommended_pkl_version();
    let compatible = get_compatible_pkl_versions();

    // Validate version format
    assert!(recommended.matches('.').count() == 2, "Version should be in x.y.z format");
    assert!(!compatible.is_empty(), "Should have at least one compatible version");
    assert!(compatible.contains(&recommended), "Recommended version should be compatible");
...
async fn test_example_config_schema_compliance() {
    ...
    let example_files = [
        ...
        ("examples/template.yml", vec!["title", "description", "variables"]),
    ];
    ...
    for (file_path, required_fields) in example_files {
        if std::path::Path::new(file_path).exists() {
            ...
            for field in required_fields {
                assert!(content.contains(&format!("{}:", field)),
                       "File {} should contain field '{}'", file_path, field);
            }
...
fn test_phase_3_implementation_completeness() {
    ...
    let example_files = ["examples/project.yml", "examples/workspace.yml",
                        "examples/toolchain.yml", "examples/template.yml"];
    ...
    let scripts = ["scripts/test-real-configs.sh", "scripts/install-pkl-version.sh"];
